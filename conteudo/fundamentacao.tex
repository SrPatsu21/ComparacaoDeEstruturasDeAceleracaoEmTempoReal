...
% build time
Tempo de construção \textit{build time}, uma das métricas mais controversas, refere-se ao tempo levado para a construção da estrutura de aceleração. Em uma simulação em tempo real, é esperado que a cena mude e, consequentemente, a estrutura de aceleração precisa ser ajustada. Existem duas abordagens principais: reconstruir (\textit{rebuild}) a estrutura todo frame e consertar (\textit{refit}) a estrutura todo frame.
