...
O \textit{Nsight Compute} permite extrair informações em nível de driver da GPU, disponibilizando métricas detalhadas para análise de desempenho de kernels CUDA \cite{nvidia_nsight_compute}.
...
Barreiras de recursos (\textit{resource barriers}) são mecanismos de sincronização utilizados para garantir a consistência no acesso a recursos compartilhados da GPU, como imagens, \textit{buffers} e estruturas de aceleração \cite{vulkan_spec}. O \textit{Nsight Graphics} é capaz de coletar chamadas relacionadas a barreiras de recursos e associá-las às respectivas filas de execução. Dessa forma, torna-se possível analisar impactos causados por sincronizações excessivas e dependências de memória \cite{nvidia_nsight_graphics_gpu_trace_ui}.
...
O processador e a memória principal do sistema desempenham papel fundamental, especialmente durante a preparação dos dados utilizados pela GPU. A CPU é responsável por tarefas como gerenciamento da cena, atualização de transformações, construção ou atualização de estruturas de aceleração e submissão de comandos gráficos e computacionais. A memória RAM armazena geometrias, materiais, descritores e demais estruturas auxiliares necessárias para a preparação dos dados antes de sua transferência para a GPU. Dessa forma, limitações de processamento da CPU ou de capacidade e largura de banda da memória principal podem introduzir gargalos que impactam o desempenho da renderização. Essas metricas pode ser adquiridas através de ferramentas de monitoramento do sistema operacional, como \textit{top}, \textit{htop} e interfaces gráficas de monitoramento do Linux, além da leitura direta das interfaces do kernel, como \texttt{/proc/stat} e \texttt{/proc/meminfo}.

O consumo energético da CPU é uma métrica relevante para análise de desempenho e eficiência computacional, principalmente em aplicações com elevada carga de processamento geométrico e gerenciamento de estruturas espaciais. Essa métrica tem muito mais relevancia quando se aplica a sistemas moveis como notebooks e celulares, que tem o tempo de uso baseado no consumo. Em sistemas Linux, esse consumo pode ser monitorado através da interface Intel Running Average Power Limit (RAPL), que disponibiliza estimativas de energia consumida pelos componentes do processador através da interface \texttt{/sys/class/powercap/intel-rapl/} \cite{hackenberg2013power} \cite{intel_rapl}.
