...
O Ray tracing é uma poderosa técnica de síntese de imagens, utilizada para renderização há décadas e que vem se tornando cada vez mais importante para aplicações em tempo real. No entanto, o tray tracing exige alto poder computacional e depende de estruturas de dados pré-processadas para alcançar um desempenho em tempo real. Dentre os métodos para melhorar a eficiência do traçado de raios, as estruturas de dados hierárquicas são extremamente importantes e populares \cite{stich2009spatial}.
...
\begin{figure}[H]
    ...
    \caption{Exemplo de Bounding Volume Hierarchy (BVH). Fonte: \cite{singleton_bvh}}
    \label{fig:bvh}
\end{figure}

Os limites de cada nó nesta árvore são escolhidos de forma que delimitem exatamente todos os nós na subárvore correspondente e que cada nó folha delimite exatamente as primitivas contidas nele. A travessia do raio começa no nó raiz e, se um raio errar um \gls{bv} nesta hierarquia, a subárvore inteira pode ser ignorada. 

Originalmente, utilizavam-se pelo menos seis bounding slabs para formar um casco fechado em torno de um objeto. 

Se essas placas forem perpendiculares ao eixo de coordenadas do mundo, o BV é chamado de \gls{aabb}. Em \glspl{bvh} padrão, os limites de um AABB são armazenados usando
seis valores de ponto flutuante.

Comparadas a outras abordagens, as \glspl{bvh}  geralmente têm os menores requisitos de memória e um memory footprint pré-computável \cite{bauszat2010minimal}.

Técnicas híbridas têm sido investigadas minuciosamente nos últimos anos, buscando combinar os benefícios das árvores kd e das \glspl{bvh}.


% oq é bounding slabs
% Como bvh viaja pelos BV traversal
