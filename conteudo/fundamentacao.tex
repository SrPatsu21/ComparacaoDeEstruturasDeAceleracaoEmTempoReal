...
A ideia por trás de uma \gls{bvh} é subdividir as primitivas de uma cena em conjuntos possivelmente sobrepostos. Para cada desses conjuntos, um \gls{bv} é calculado e estes são organizados em uma estrutura de árvore. Os limites de cada nó nesta árvore são escolhidos de forma que ele delimite exatamente todos os nós na subárvore correspondente e cada nó folha delimite exatamente as primitivas contidas.

A travessia do raio começa então no nó raiz e, se um raio não encontrar um \gls{bv} nesta hierarquia, a subárvore inteira pode ser ignorada. Comparadas a outras abordagens, as \glspl{BVHs}  geralmente têm os menores requisitos de memória e um memory footprint pré-computável \cite{bauszat2010minimal}.
