...
Espera-se, ao final deste trabalho, o desenvolvimento de uma aplicação capaz de realizar testes comparativos entre diferentes implementações de \glspl{bvh} em um ambiente de renderização híbrida utilizando técnicas básicas de \textit{ray tracing}. A aplicação deverá permitir a execução controlada de experimentos, coleta de métricas de desempenho e análise do comportamento das estruturas em diferentes tipos de cena e condições de hardware.

Também identificação das características, vantagens e limitações de cada algoritmo de \gls{bvh} analisado. A partir dos dados obtidos, pretende-se determinar quais estruturas apresentam melhor desempenho em cenários específicos, considerando fatores como quantidade de objetos dinâmicos, complexidade geométrica da cena, custo de atualização, consumo de memória e utilização da \gls{gpu} e da \gls{cpu}.
