...
Diante disso, surge o seguinte problema de pesquisa: qual abordagem de construção e atualização de uma \gls{bvh} apresenta melhor desempenho em simulações de tempo real, considerando diferentes cenários de hardware dedicado e cenas dinâmicas?
...
\begin{itemize}
    \item Analisar as características de diferentes estruturas \gls{bvh} em simulações de tempo real;
    ...
    \begin{itemize}
        \item Latência por quadro (\textit{frame});
        \item Uso de memória;
...
Além disso, abordagens como \textit{rebuild} completo e \textit{refit} incremental apresentam características distintas, que podem influenciar diretamente a eficiência da aplicação dependendo do cenário de uso. A análise desses fatores é importante não apenas do ponto de vista teórico, mas também prático, contribuindo para a escolha de técnicas mais adequadas para aplicações reais.
